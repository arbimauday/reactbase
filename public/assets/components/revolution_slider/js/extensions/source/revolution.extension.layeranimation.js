...
jQuery.extend(true,_R, {
	...
	***************************************/
	...
	/*	if (_nc.hasClass("tp-videolayer") &&  _nc.dataview('videoposter')!=undefined && (_nc.dataview('noposteronmobile')=="on" && _ISM)) {
			var vidw =  makeArray(_nc.dataview('videowidth'),opt)[opt.curWinRange] || makeArray(_nc.dataview('videowidth'),opt) || "auto",
				vidh =  makeArray(_nc.dataview('videoheight'),opt)[opt.curWinRange] || makeArray(_nc.dataview('videoheight'),opt) || "auto";
			
			vidw = parseFloat(ww),
			vidh = parseFloat(hh);
			
			_nc.append('<div class="tp-videoposter" style="position:absolute;top:0px;left:0px;width:100%;height:100%;background-image:url('+_nc.dataview('videoposter')+'); background-size:cover;background-position:center center;"></div>');
			if (vidw!="100%") 
...
/*					punchgs.TweenLite.delayedCall(parseInt(_nc.dataview('end'),0)/1000,_R.endMoveCaption,[_nc,_mw,_pw,opt],opt.sliderscrope);
				else
